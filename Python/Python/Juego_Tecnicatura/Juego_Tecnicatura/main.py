...
def main() :
    ...
    sounds_path = os.path.join(base_path, 'assets_1', 'sounds')

    # Actualizar las rutas para reflejar las ubicaciones correctas
    imagen_inicial_path = os.path.join(images_path, 'icon.png')
    ...
    fondo1 = pygame.image.load(os.path.join(images_path, 'fondo1.jpg'))
    fondo1 = pygame.transform.scale(fondo1, (SCREEN_WIDTH, SCREEN_HEIGHT))

    fondo2 = pygame.image.load(os.path.join(images_path, 'fondo2.jfif'))
    fondo2 = pygame.transform.scale(fondo2, (SCREEN_WIDTH, SCREEN_HEIGHT))

    fondo3 = pygame.image.load(os.path.join(images_path, 'fondo3.jpg'))
    fondo3 = pygame.transform.scale(fondo3, (SCREEN_WIDTH, SCREEN_HEIGHT))

    # Cambiar el nombre del archivo de sonido a 'laserdis.mp3'
    sonido_laser = pygame.mixer.Sound(os.path.join(sounds_path, 'laserdis.mp3'))
    sonido_explosion = pygame.mixer.Sound(os.path.join(sounds_path, 'explosion.mp3'))

    # Reproducir sonido de fondo
    pygame.mixer.music.load(os.path.join(sounds_path, 'efectos.mp3'))
    ...
    # Inicializar el fondo actual con fondo1
    fondo_actual = fondo1
    ...
    while running :
        ...
        # Variable para controlar la frecuencia de disparo
        tiempo_ultimo_disparo = 0
        intervalo_disparo = 300  # Milisegundos entre disparos
        ...
        for enemigo in enemigos [ : ] :  # Iterar sobre una copia para eliminar de la lista original
            ...
            for laser in personaje.lasers[:]:  # Iterar sobre una copia para eliminar de la lista original
                if enemigo.rect.colliderect(laser.rect):
                    ...
                    puntos += 10  # Incrementar el puntaje al destruir un enemigo
                    break  # Salir del bucle para evitar errores

            # Incrementar puntaje al esquivar enemigos
            if enemigo.rect.top > SCREEN_HEIGHT:
                puntos += 5  # Incrementar el puntaje al esquivar un enemigo
